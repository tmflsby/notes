...
- [14. 盒子模型（CSS重点）](#14-盒子模型css重点)
  - [14.1. `border`盒子边框](#141-border盒子边框)
...
- [23. CSS高级技巧](#23-css高级技巧)
  ...
  - [23.2. `vertical-align` 垂直对齐](#232-vertical-align-垂直对齐)
    - [23.2.1. 图片、表单和文字对齐](#2321-图片表单和文字对齐)
...
链接式是将所有的样式放在一个或多个以`.CSS`为扩展名的外部样式表文件中，通过`link`标签将外部样式表文件链接到`HTML`文档中，其基本语法格式如下：
...
<!-- ![box-shadow](./media/1498467567011.png) -->
...
浮动的元素排列位置，跟上一个元素（块级）有关系。`如果上一个元素有浮动`，则A元素顶部会和上一个元素的`顶部对齐`；如果上一个元素是标准流，则A元素的顶部会和上一个元素的底部对齐。
...
`W3C推荐的做法:通过在浮动元素末尾添加一个空的标签例如 <div style=”clear:both”></div>，或则其他标签br等亦可。`
...
测试文字ing<img src="./media/1633.png"  width="500"  style="border: 1px dashed #ccc;  vertical-align: top" />
...
`white-spance: normal | nowrap`  设置或检索对象内文本显示方式。  
`normal` : 　默认处理方式  
...
上传完毕， 网站会把`svg图片转换为字体格式`， 然后下载下来就好了

当然，我们不需要自己专门的图标，是想网上找几个图标使用，以上2步可以直接省略了， 直接到刚才的网站上找喜欢的下载使用吧。

<img src="./media/fontt1.png" width="600px" />

<img src="./media/fontt2.png" width="600px" />

### 28.2.4. 字体引入到HTML

得到压缩包之后，最后一步，是最重要的一步了， 就是字体文件已经有了，我们需要引入到我们页面中。

将以下4个文件放入到 fonts文件夹里面。 通俗的做法

<img src="./media/1498032122244.png">

#### 28.2.4.0.1. 第一步：在样式里面声明字体
...
`font-family: 'icomoon';` 给字体起的名字  
`font-style: normal;` 倾斜字体正常  
注意：`url地址`为当前css文件到fonts文件夹的路径，`?`后面的可以去掉。

#### 28.2.4.0.2. 第二步：盒子使用字体
...
现阶段电商类网站很流行，这里用京东电商网站复习、总结、提高前面所学布局技术。
...
SEO是由英文`Search Engine Optimization`缩写而来， 中文意译为“`搜索引擎优化`”！SEO是指通过对网站进行站内优化、网站结构调整、网站内容建设、网站代码优化等)和站外优化，从而提高网站的关键词排名以及公司产品的曝光度。 简单的说就是，把产品做好，搜索引擎就会介绍客户来。  
...
#### 29.6.3.3. Keywords 关键字

Keywords是页面关键词，是搜索引擎关注点之一。`Keywords`应该限制在`6～8个关键词`左右，电商类网站可以多 少许。
...
| `logo优化`     | `text-indent: -20000px;` 或者 `font-size:0;` 隐藏文字， 只显示背景图片                   |
...
`rgba的alpha 和 元素的opacity属性，取值都为 0~1 ，越小越透明。`

## 30.4. 背景渐变 `linear-gradinet`
...
```css
background:-webkit-linear-gradient(渐变的起始位置， 起始颜色，颜色，...， 结束颜色)；

background: linear-gradinet(to top, red 0%, green 50%, blue 100%);
```
...
### 31.3.1. datalist 选项列表

- datalist   标签定义选项列表。请与 input 元素配合使用该元素

```html
<!--  input里面用 list属性关联datalist的id -->
...
## 31.4. 新增的input type属性值：

| `类型`         | `使用示例`   |  `显示`   | `含义`         |
| ---------------- | -------------- | --------- | -------------------- |
| `email`    | `<input type="email">`    | <input type="email">    |  输入邮箱格式         |
| `tel`      | `<input type="tel">`      | <input type="tel">      |  输入手机号码格式     |
| `url`      | `<input type="url">`      | <input type="url">      |  输入url格式          |
| `number`   | `<input type="number" min="1" max="10">`   | <input type="number" min="1" max="10"> | 输入数字格式 |
| `search`   | `<input type="search">`   | <input type="search">   |  搜索框（体现语义化） |
| `range`    | `<input type="range min="1" max="10"">`    | <input type="range" min="1" max="10"> |  自由拖动滑块 |
| `date`     | `<input type="date">`     | <input type="date">     |  年月日               |
| `month`    | `<input type="month">`    | <input type="month">    |  月年                 |
| `week`     | `<input type="week">`     | <input type="week">     |  年 第几周            |
| `time`     | `<input type="time">`     | <input type="time">     |  小时分钟             |
| `datetime` | `<input type="datetime">` | <input type="datetime"> |  时间、日、月、年     |
| `datetime-local` | `<input type="datetime-local">` |<input type="datetime-local"> | 时间、日、月、年（本地时间） |

## 31.5. 常用新属性

| `input属性`           | `用法`      |    `显示样式`              | `含义`                               |
| -------------------- | ------------------ | ---------------------- | ---------------------------------------- |
| `placeholder`  | `<input type="text" placeholder="请输入用户名">` | <input type="text" placeholder="请输入用户名"> | 占位符  当用户输入的时候 里面的文字消失  删除所有文字，自动返回       |
| `autofocus`    | `<input type="text" autofocus>`  | 页面加载完获取焦点，不便演示      | 规定当页面加载时 input 元素应该自动获得焦点                |
| `multiple`     | `<input type="file" multiple>` | <input type="file" multiple>            | 多文件上传。不加multiple即默认单文件上传。                                    |
| `autocomplete` | `<input type="text" autocomplete="off">` | <input type="text" autocomplete="off">   | 规定表单是否应该启用自动完成功能  有2个值，on 和 off。on 代表记录已经输入的值。 <br> 1.autocomplete 首先需要提交按钮 <br/>2.这个表单您必须给他名字 |
| `required`     | `<input type="text" required>` | <input type="text" required>        | 必填项。内容不能为空                              |
| `accesskey`    | `<input type="text" accesskey="s">` | <input type="text" accesskey="s">       | 规定激活（使元素获得焦点）元素的快捷键   采用 alt + s的形式      |
...
```html
<audio src="./music/See You Again.mp3" autoplay controls></audio>
```
...
<img src="./media/1498468041058.png" height="160px">
...
```html
<video source src="./video/movie.mp4"></video>
```
...
<img src="./media/1498468086199.png" height="160px">
...
## 32.1. 结构(位置)伪类选择器（CSS3)

- `:first-child` :选取属于其父元素的首个子元素的指定选择器
- `:last-child` :选取属于其父元素的最后一个子元素的指定选择器
- `:nth-child(n)` ： 匹配属于其父元素的`第 N 个子元素`，不论元素的类型
- `:nth-last-child(n)` ：选择器匹配属于其元素的`倒数第 N 个子元素`。不论元素的类型，`从最后一个子元素开始计数`。
  n 可以是数字、关键词或公式
- `n` 的取值：  `2n`、`even`、`odd`、`(2n+1)`、`n` ...
- `div:nth-of-type(n)` :匹配div兄弟div元素中的第n个。

```css
/* 注意：first-child 只有在它的父元素的第一个子元素是它时，才起作用，否则无效。 */
...
`:target` 目标伪类选择器 :选择器可`用于选取当前活动的目标元素` 有href的元素
...
补充：  
`::selection 伪元素`应用于`文档中`被用户高亮的部分（比如使用鼠标或其他选择设备选中的部分）  
...
<style>
/* 获取到 拥有 该属性的元素 */
  div[class^=font] { /*  class^=font 表示 以 font开始的 class */
    color: pink;
  }
  div[class$=footer] { /*  class$=footer 表示以 footer结束的 class */
    color: skyblue;
  }
  div[class*=tao] { /* class*=tao  *=  表示带有 tao 的class */
    color: green;
  }
</style>
...
`::before` 和 `::after`  在`元素内部的开始位置和结束位置`创建一个元素，该元素为`行内元素`，且必须要结合content属性使用。
...
`伪元素::before和::after添加的内容默认是inline元素`；这个两个伪元素的`content`属性，表示伪元素的内容,设置`::before`和`::after`时必须设置其`content`属性，否则伪元素就不起作用。
...
2. 设置为`cover`时，会自动调整缩放比例，保证图片始终`填充`满背景区域，如有溢出部分则会被隐藏。我们平时用的cover 最多

3. 设置为`contain`会自动调整缩放比例(`拉伸`)，保证图片始终完整显示在背景区域。
...
以`逗号`分隔可以设置多背景，可用于自适应布局  做法就是 用逗号隔开就好了。

- 一个元素可以设置多重背景图像。 
- 每组属性间使`用逗号分隔`。 
- 如果设置的多重背景图之间存在着交集（即存在着重叠关系），前面的背景图会覆盖在后面的背景图之上。(`前者覆盖后者`)
...
`text-shadow: 水平距离  垂直距离  模糊距离 阴影颜色;`设置文本阴影。
...
<div style="background-color: #808080;color:#808080; width:650px; font: 700 80px '微软雅黑'">
  <div style="text-align:center; text-shadow: 1px 1px 1px #000, -1px -1px 1px #fff;">我是凸起的文字</div>
  <div style="text-align:center; text-shadow: -1px -1px 1px #000, 1px 1px 1px #fff;">我是凹下的文字</div>
</div>
...
content-box:为其默认值，让元素维持W3C的标准Box Mode

2、`box-sizing: border-box`  盒子大小为 width    就是说  padding 和 border 是包含到width里面的 (`内减`)
...
## 32.9. `transition`过渡(CSS3)

`过渡`（`transition`)是CSS3中具有颠覆性的特征之一，我们可以在不使用 Flash 动画或 JavaScript 的情况下，当元素从一种样式变换为另一种样式时为元素添加效果。

`帧动画`：通过一帧一帧的画面按照固定顺序和速度播放。如电影胶片

在CSS3里使用`transition`可以实现补间动画（过渡效果），并且当前元素只要有“属性”发生变化时即存在两种状态(我们用A和B代指），就可以实现平滑的过渡，为了方便演示采用hover切换两种状态，但是并不仅仅局限于hover状态来实现过渡。

语法格式:
- `transition: transition-property transition-duration transition-timing-function transition-delay;`

- `要过渡的属性`  `花费时间`  `运动曲线`  `何时开始`;
如果有多组属性变化，还是用逗号隔开。`后两个参数可以省略`。
...
如果想要所有的属性都变化过渡， `transition-property` 写一个 `all` 就可以

`transition-duration`  `花费时间` 比如 0.5s    这个s单位必须写  

`transition-timing-function` `运动曲线 默认ease`  
...
div {
  ...
  /* transition 写在hover里面，鼠标离开时瞬间失去过渡效果 */
}
...
## 32.10. `transform` 2D变形(CSS3) 

`transform`是CSS3中具有颠覆性的特征之一，可以实现元素的位移、旋转、倾斜、缩放，甚至支持矩阵方式，配合过渡和即将学习的动画知识，可以取代大量之前只能靠Flash才可以实现的效果。

变形转换 transform    transform  变换 变形的意思             《 transformers 变形金刚》

### 32.10.1. `translate(x,y)`移动
...
`scaleY(y)`元素仅垂直方向缩放（Y轴缩放）
 ...
 `scale()`的取值`默认值为1`，当值设置为`0.01~0.99`之间的任何值，作用使一个元素缩小；而任何大于或等于1.01的值，作用是让元素放大

### 32.10.3. `rotate(deg)`旋转

可以对元素进行旋转，正值为顺时针，负值为逆时针；

<img src="./media/1498443651293.png">
...
### 32.10.4. `transform-origin` `调整` `元素` `旋转变换` `的原点`

<img src="./media/1498443912530.png">
...
<img src="./media/1498459001951.png">

简单记住他们的坐标：(`立方体的 左上里角 为原点`)
x左边是负的，右边是正的  
y 上面是负的， 下面是正的  
z 里面是负的， 外面是正的  

**设置以3d样式做过渡：** `transform-style: flat|preserve-3d;`

### 32.11.1. `rotateX()`

 就是沿着 x 立体旋转.

<img src="./media/1498445756802.png">
...
<img src="./media/1498446043198.png">
...
- perspective：视距，表示视点距离屏幕的长短(px,rem,cm,none)。视点，用于模拟透视效果时人眼的位置
...
<img src="./media/1498446715314.png">

### 32.11.5. `translateX(x)`

仅水平方向移动`（X轴移动）

<img src="./media/1498459697576.png">

主要目的实现移动效果

### 32.11.6. `translateY(y)`

仅垂直方向移动（Y轴移动）

<img src="./media/1498459770252.png">

### 32.11.7. `translateZ(z)`

transformZ的直观表现形式就是大小变化(`近大远小`)，实质是XY平面相对于视点的远近变化（说远近就一定会说到离什么参照物远或近，在这里参照物就是perspective属性）。比如设置了perspective为200px;那么transformZ的值越接近200，就是离的越近，看上去也就越大，超过200就看不到了，因为相当于跑到后脑勺去了，我相信你正常情况下，是看不到自己的后脑勺的。

### 32.11.8. `translate3d(x,y,z)`
...
### 32.11.10. `backface-visibility` 背向屏幕
...
div img:first-child {
  z-index: 1;
  backface-visibility: hidden; /* 不是正面朝屏幕，就隐藏 */
}
div:hover img {
  transform: rotateY(180deg);
}
...
<img src="./media/1498461096243.png" width="850px">

关于几个值，除了`名字`，`动画时间`，`延时`有严格顺序要求,其它随意。
...
<img src="./media/1498441839910.png">
...
3、`justify-content`设置`主轴对齐`

子盒子如何在父盒子里面水平对齐

| 值             | 描述                       | 白话文                     |
| ------------- | ------------------------ | ----------------------- |
| `flex-start`   | 默认值。项目位于`容器的开头`。           | 让子元素从父容器的开头开始排序但是盒子顺序不变 |
| `flex-end`      | 项目位于`容器的结尾`。               | 让子元素从父容器的后面开始排序但是盒子顺序不变 |
| `center`        | 项目位于`容器的中心`。               | 让子元素在父容器中间显示            |
| `space-between` | `项目之间留有空白`。        | 左右的盒子贴近父盒子，中间的平均分布空白间距  |
| `space-around`  | `项目之前、之间、之后都留有空白`。 | 相当于给每个盒子添加了左右margin外边距  |
| `space-evenly`  | `项目平均分布`。 | 相当于每个盒子左右间距一样  |

4、`align-items`设置`侧轴对齐`
...
`align-content`是针对flex容器里面多轴(`多行`)的情况,
`align-items`是针对`一行`的情况进行排列。
...
<img src="./media/1498467502625.png" width="800">
...
<div style="font: bold 4em 'microsoft yahei'; text-shadow: 3px 4px 5px rgba(0, 0, 0, 0.5)">我是带有影子的文字</div>
...
`display` 为 `inline-block`, `table-cell`, `table-caption`, `flex`, `inline-flex`,`block`, `list-item`, `table`  
`overflow` `不为visible`。
...
2. 盒子垂直方向的距离由`margin`决定。属于同一个BFC的两个相邻盒子的margin会发生重叠。
...
主要用到: 普通流元素BFC后，为了和浮动元素不产生任何交集，顺着浮动边缘形成自己的封闭上下文
